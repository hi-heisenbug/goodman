...
func TestAuthRejectsAndAccepts(t *testing.T) {
	...
	cases := []struct {
		...
		{"snapshot no token", http.MethodGet, "/v1/snapshot", "", "", http.StatusUnauthorized},
		{"snapshot ingest token rejected", http.MethodGet, "/v1/snapshot", "Bearer ingest-secret", "", http.StatusUnauthorized},
		{"snapshot right token", http.MethodGet, "/v1/snapshot", "Bearer api-secret", "", http.StatusOK},
		{"state export no token", http.MethodGet, "/v1/export", "", "", http.StatusUnauthorized},
		{"state export ingest token rejected", http.MethodGet, "/v1/export", "Bearer ingest-secret", "", http.StatusUnauthorized},
		{"state export right token", http.MethodGet, "/v1/export", "Bearer api-secret", "", http.StatusOK},
