...
## Phase 8: Pilot heartbeat (scheduled reachability + weekly digest) (DONE)

The strategic frame above names the risk: a silent 30-day POV reads as "it
does nothing." Make the product speak on a schedule from day one. Shipped:

- previous-snapshot columns on `reachability_reports`; `GET /v1/report` returns
  a `delta` (executed / declared / reachable vulns + newly executed packages)
  so the Reachability tab shows "+N vs prior" trend details
- weekly digest (`internal/digest`, `-digest-interval` / Helm
  `notifications.digestInterval=168h`): open alerts vs budget, coverage, new
  executed packages and reachable CVEs; delivered via the existing webhook
  once at startup and then on cadence
- Slack alert formatting with matched rules, per-behavior sensor + first-seen,
  and a deep link when `-public-url` / `publicUrl` is set
- Helm wires `reachability.interval`, digest, and `publicUrl`

**DoD:** a fresh install with a webhook configured emits its first digest with
zero manual steps; the Reachability tab shows a trend, not just a point-in-time
